...
def preview_influx(st,influx_server,influx_user_login,influx_password,influx_database,influx_port,column_names,mqtt_topic) :
      try:
            ...
            if mqtt_topic.split('/')[0] =='data':
                query1 = f"select time,topic,model,lot,d_str1,d_str2,{column_names} from mqtt_consumer where topic = '{mqtt_topic}' order by time desc limit 5"
...
def config_sensor_registry_add():
    ...
    with st.form("config_sensor_registry_add"):
        ...
        table_column_value = "registered_at datetime,mc_no varchar(10),process varchar(10),model varchar(25),lot varchar(25),d_str1 varchar(15),d_str2 varchar(15)"
...
def main_layout():
    st.set_page_config(
            page_title="MMS Sidelap Config 1.0.0",
            page_icon="🗃",
            layout="wide",
